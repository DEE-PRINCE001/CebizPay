...
public sealed class LedgerPostingService : ILedgerPostingService
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (_dbContext.Database.CurrentTransaction != null)
        {
            return await PostSingleCurrencyTransactionCoreAsync(
                sourceLedgerAccountId, targetLedgerAccountId, amount, currency, transactionType, reference, idempotencyKey, description, cancellationToken).ConfigureAwait(false);
        }

        return await _dbContext.ExecuteInTransactionAsync(async ct =>
        {
            return await PostSingleCurrencyTransactionCoreAsync(
                sourceLedgerAccountId, targetLedgerAccountId, amount, currency, transactionType, reference, idempotencyKey, description, ct).ConfigureAwait(false);
        }, cancellationToken).ConfigureAwait(false);
    }

    private async Task<LedgerTransaction> PostSingleCurrencyTransactionCoreAsync(
        Guid sourceLedgerAccountId,
        Guid targetLedgerAccountId,
        decimal amount,
        Currency currency,
        ...
        CancellationToken cancellationToken = default)
    {
        if (_dbContext.Database.CurrentTransaction != null)
        {
            return await PostCrossCurrencyTransactionCoreAsync(
                sourceWalletId, targetWalletId, sourceAmount, targetAmount, rate, rateProvider, rateTimestamp, reference, idempotencyKey, description, cancellationToken).ConfigureAwait(false);
        }

        return await _dbContext.ExecuteInTransactionAsync(async ct =>
        {
            return await PostCrossCurrencyTransactionCoreAsync(
                sourceWalletId, targetWalletId, sourceAmount, targetAmount, rate, rateProvider, rateTimestamp, reference, idempotencyKey, description, ct).ConfigureAwait(false);
        }, cancellationToken).ConfigureAwait(false);
    }

    private async Task<(LedgerTransaction Transaction, FxConversion FxRecord)> PostCrossCurrencyTransactionCoreAsync(
        Guid sourceWalletId,
        Guid targetWalletId,
        decimal sourceAmount,
        decimal targetAmount,
        ...
        CancellationToken cancellationToken = default)
    {
        if (_dbContext.Database.CurrentTransaction != null)
        {
            return await ReverseTransactionCoreAsync(originalTransactionId, reason, cancellationToken).ConfigureAwait(false);
        }

        return await _dbContext.ExecuteInTransactionAsync(async ct =>
        {
            return await ReverseTransactionCoreAsync(originalTransactionId, reason, ct).ConfigureAwait(false);
        }, cancellationToken).ConfigureAwait(false);
    }

    private async Task<LedgerTransaction> ReverseTransactionCoreAsync(
        Guid originalTransactionId,
        string reason,
        CancellationToken cancellationToken)
    {
