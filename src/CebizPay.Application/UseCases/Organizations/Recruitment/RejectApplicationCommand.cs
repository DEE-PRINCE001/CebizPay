...
{
    /// <summary>
    /// Initializes validation rules for RejectApplicationCommand.
    /// </summary>
    public RejectApplicationCommandValidator()
    {
        ...
        RuleFor(x => x.RejectionReason).NotEmpty().WithMessage("Rejection reason is required.").MaximumLength(1000);
...
/// </summary>
public sealed class RejectApplicationCommandHandler : IRequestHandler<RejectApplicationCommand, bool>
{
    ...
    /// <summary>
    /// Initializes a new instance of <see cref="RejectApplicationCommandHandler"/>.
    /// </summary>
    ...
    public async Task<bool> Handle(RejectApplicationCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Cannot reject applications while organization status is suspended.");
        }
        ...
        application.Reject(actorUserId, request.RejectionReason, now, request.Notes);

        var auditLog = AuditLog.Create(
            actorId: actorUserId,
            action: AuditActions.ApplicationRejected,
        ...
        _outboxService.Write(new RecruitmentApplicationRejectedDomainEvent(
            application.Id,
            application.JobPostingId,
